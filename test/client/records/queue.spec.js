    ...
    describe( 'storing records', () => {
        ...
        it( 'creates a last-saved record when updating a record', done => {
            ...
            records.save( 'set', recordA )
                .then( () => {
                    ...
                    return store.record.remove( lastSaved.getLastSavedInstanceId( enketoId ) );
                } )
                .then( () => {
        ...
        it( 'creates separate last-saved records for different forms', done => {
            ...
            records.save( 'set', recordA )
                ...
                .catch( err => {
                    console.log( 'err', errCount++, err, err.stack );
                ...
                .then( lastSavedB => {
                    Object.entries( originalRecordB ).forEach( ( [ key, value ] ) => {
                        if ( key === 'instanceId' ) {
                            expect( lastSavedB[key] ).to.equal( lastSaved.getLastSavedInstanceId( enketoId ) );
                        } else if ( key === 'name' ) {
                ...
                .then( lastSavedA => {
                    Object.entries( originalRecordA ).forEach( ( [ key, value ] ) => {
                        if ( key === 'instanceId' ) {
                            expect( lastSavedA[key] ).to.equal( lastSaved.getLastSavedInstanceId( enketoId ) );
                        } else if ( key === 'name' ) {
